...
def build_custom_table(df):
    # (Logic copied from your notebook and cleaned up)
    ...
    result["ကွပ်ကဲမှု"] = df["ကွပ်ကဲမှု့"] # Note: source uses 'ကွပ်ကဲမှု့'
...
def process_data(input_file_path, output_folder, wanted_date_str, font_path, wkhtmltopdf_path):
    ...
    # --- Logic 2: Admitted Patients (cols isna) ---
    ...
    # 1. Generate Custom Table (Main List)
    ...
    # 2. Pivot: Diagnosis (SaSaYa vs Other)
    if not admitted_patients.empty:
    ...
    # 3. Pivot: Room
    if not admitted_patients.empty and 'room' in admitted_patients.columns:
        # Note: Your notebook used 'ID' for counting in the room function, but 'ကိုယ်ပိုင်အမှတ်' elsewhere.
        # Assuming 'ကိုယ်ပိုင်အမှတ်' is the ID to count.
